...
-}
sumLast2 :: Int -> Int
sumLast2 n = fstDigit + sndDigit
  where
    twoDigits = abs n `mod` 100
    fstDigit = twoDigits `mod` 10
    sndDigit = twoDigits `div` 10
...
firstDigit :: Int -> Int
firstDigit n 
  | droppedLastDigit == 0 = abs n
  | otherwise = firstDigit droppedLastDigit
   where 
     droppedLastDigit = abs n `div` 10
